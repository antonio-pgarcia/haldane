...
public class BacteriumEquations {
	...
	/**
	 * Sample a normally distributed random variable with specified mean and standar deviation
	 * 
	 * @param avg		The average of random variable
	 * @param sigmaM	The standard deviation 
	 * @param Z			Sampled random variable from a standard normal distribution
	 * @return V		The normally distributed value
	 */
	public static double eqnZ(double avg, double sigma, double Z) {
		double v= sigma * Z + avg;
		return v;
	}
	
	public static double eqnSimpleLinearFit(double C, double E, double min, double max) {
		double v= min + ( (max - min) * C/E);
		return v;
	}
	...
	public static double getLagFromG(double v) {
		return Math.exp(2.05 + 0.91 * Math.log(v));
	}
}
